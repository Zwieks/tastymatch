...
<form class="page-searchbox" id="js-page-searchbox" method="get" action="{{url(Lang::get('menus.search-url'))}}" itemprop="potentialAction" itemscope itemtype="http://schema.org/SearchAction">
    ...
    <input  class="form-control" id="search-bar" type="search" name="q" autocomplete="off" value="{{  isset($search) ? $search : old("q") }}" placeholder="{{ Lang::get('forms.searchplaceholder') }}" itemprop="query-input">
